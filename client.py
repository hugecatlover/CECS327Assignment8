...
def tcp_client():
    ...
    while len(message) != 0:                              # Continue sending messages until the user inputs an empty string
        client_socket.send(message.encode())              # Sending the message to the server, encoded as bytes
        print("Sending message to server...")             # Notify when message is being sent

        # the server recieves the response
        server_response = client_socket.recv(1024).decode()  # Receive the response from the server and decode it
        print(f"Message from server: {server_response}")  # Print the server's response which would be capital letter message

        # Ask for another message, or an empty input to stop sending
        message = input("What is another message you want to send? (enter nothing if you don't want to send anymore) ")
